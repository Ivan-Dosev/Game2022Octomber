...
struct PipeFactory {
    ...
    static func launch(for scene: CGSize, targetNode: SKNode) -> SKAction {
        let pipeName = "pipe"
      ...
      //  let waitAction = SKAction.wait(forDuration: UserDefaults.standard.getDifficultyLevel().rawValue)
        ...
        let pipeMoveDuration: TimeInterval = 9.5

        let producePipeAction = SKAction.run {
            
            guard var pipe = PipeFactory.producseDoublePipe(sceneSize: scene) else {
                print("dddddd....")
                return
            }
            if Bool.pseudoRandomPipe {
                guard let standardPipe = PipeFactory.produceStandardPipe(sceneSize: scene) else {
                    print("mmmmmm....")
                    return
                }
                pipe = standardPipe
            ...
            pipe.name = pipeName
            pipe.position = CGPoint(x: scene.width  , y: 0)
            
            targetNode.addChild(pipe)
            
            let moveAction = SKAction.move(to: CGPoint(x: -(pipe.size.width + scene.width), y: pipe.position.y), duration: pipeMoveDuration)
    ...
    private static func standardPipeParts(for sceneSize: CGSize) -> PipeParts? {
      ...
      //  pipeBottom?.size = pipeBottomSize
